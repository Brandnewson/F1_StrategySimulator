...
\markboth{APPENDIX B. EXTERNAL MATERIAL}{APPENDIX B. EXTERNAL MATERIAL}
...
\label{appendix:litreview-method}
...
\section{Databases and inclusion criteria}
...
The structured comparison of the most directly related studies, organised by the variables they manipulate and the failure modes they engage with, is in Appendix Table~\ref{tab:related-work-comparison}.
...
\hline
A & \texttt{test\_inactive\_components\_zero}\textsuperscript{*} & \texttt{pace} and \texttt{tyre\_pit} components are exactly 0.0 in every event at low complexity & Pass \\
\hline
...
\section{Phase 10: Mean-field difference reward derivation}
\label{appendix:difference-rewards}
...
where $\Delta_i$ is the position delta for agent $i$ and $\bar{\Delta}_{\text{team}}$ is the mean position delta across the $N$ DQN agents on the team. Expanding the expression gives

\[
R_{\text{diff},i} = 2\Delta_i - \frac{1}{N}\sum_{j=1}^{N}\Delta_j = \frac{2N-1}{N}\Delta_i - \frac{1}{N}\sum_{j \neq i}\Delta_j
\]

which makes the structural properties used in the Chapter~\ref{chapter4} analysis visible. The coefficient on $\Delta_i$ is $(2N-1)/N$, amplified relative to identity, and the coefficient on each teammate's $\Delta_j$ is $-1/N$, which is negative. This is the formulation analysed in Section~\ref{sec:rq4}.
...
\begin{tabular}{@{}ll@{}}
\textbf{CPU}         & AMD Ryzen 5 2600 (6 cores, 12 threads, 3.4\,GHz base) \\
\textbf{GPU}         & NVIDIA GeForce RTX 2060 (6\,GB GDDR6) \\
\textbf{Memory}      & 32\,GB DDR4-3200 (G.Skill Trident Z) \\
\textbf{Motherboard} & ASUS ROG Strix B450-F \\
\end{tabular}
...
\chapter{Engineering log}
\label{appendix:engineering-log}

This appendix presents the full Git commit graph for the project repository, generated with \texttt{git log --graph --oneline --all}. The history shows the phase-numbered feature branches that correspond to the sprint structure in Section~\ref{sec:engineering}, alongside the supporting infrastructure work (simulator construction, DQN substrate tuning, autoresearch hyperparameter exploration) that preceded the formal phase numbering.

\begin{small}
\begin{verbatim}
*   390cc2b (HEAD -> main, origin/main, origin/HEAD) Merge pull request #1 from Brandnewson/develop
|\  
| *   6f9a0ca (origin/develop) Merge branch 'develop' of https://github.com/Brandnewson/F1_StrategySimulator into develop
| |\  
| |/  
|/|   
* | 63f6ce8 (develop, claude/blissful-cray-533300) implemented batch and real-time simulator logic, with scope for implementing the AI agents
...
| * deeaaed simplification and condensation of words
...
| * 678edc1 fix: Task 9 style pass ΓÇö remove prose colons, semi-colons, em-dashes and flagged violations
| * 99ae46c feat: rewrite Appendix A self-appraisal and Appendix B external materials
| * 936f3b3 feat: Chapter 4 sections 4.4-4.7 ΓÇö RQ3, discussion, validity, future work
...
| * 1b6227f feat: Chapter 4 Section 4.3 ΓÇö RQ2 results covering Phases 4-8
| * c9a5f8d feat: Chapter 4 sections 4.1 and 4.2 ΓÇö preliminary validation and RQ1
| * 6dd5b0f fix: remove colon and semi-colons from Chapter 3 Phase 0 defects sentence
| * d6a5cfa feat: fix Chapter 3 placeholder, tighten Phase 0 reference, add Section 3.8 for Phase 4-10 infra
...
| * 40cf51b (origin/phase-8-sweeping-alphas-training) Update model weights and add full research journey documentation
| * ec9eb3e (origin/phase-7-credit-assignment-boundary) phase 7 omplete
| * 4b4b3d2 (origin/phase-6-MARK-intrateam-dynamics, phase-6-MARK-intrateam-dynamics) phase 6 completion
...
| * 016de6b (origin/simplicity-experiment) Adjust graph to correctly reflect overtakes instead of absolute value as before
...
| *   3c390ae Merge branch 'dqn-experiment' of https://github.com/Brandnewson/F1_StrategySimulator into dqn-experiment
| |\  
| | * b320d75 Attempt 1: Slower epsilon decay allows the agent to explore longer duri
| * | dda182c Attempt 28: Current best score (0.716667) was achieved with learning_rat
| * | c247f49 Attempt 12: Increasing buffer capacity will allow the agent to retain mo
| * | 9d972c4 Attempt 8: Increasing network capacity from 256 to 512 hidden units wil
| * | 78f780c Attempt 7: Even slower epsilon decay will allow the agent to maintain e
| * | 6672323 Attempt 6: Current learning rate may be too conservative for convergenc
| |/  
...
| * 703d777 Attempt 26: Increasing target network update frequency will reduce Q-val
| * 95c64f8 Attempt 21: Lower epsilon_min will force more aggressive exploitation in
| * aa2ee3d Attempt 20: Larger replay buffer will capture more diverse racing scenar
| * 2e31067 Attempt 10: Increasing epsilon_min from 0.02 to 0.05 will maintain more
| * b2e55c3 Attempt 6: Larger replay buffer capacity allows the agent to retain mor
| * 88489ab Attempt 2: Increasing hidden layer capacity allows the DQN to learn mor
| * 7e13ed1 Attempt 1: Slower epsilon decay allows the agent to explore more thorou
| * d106d4e Attempt 1: Slower epsilon decay will maintain exploration longer during
| * 7f4f11c Attempt 1: Slower epsilon decay allows the agent to explore more thorou
| * 248765c test research on just 1 base agent, vs DQN agent
| * 7d63d7a (origin/dev/bt/DQN-shaping, origin/autoresearch/0314) Attempt 29: Lowering epsilon_min from 0.05 to 0.02 reduces residual expl
...
| * c4f047b (origin/autoresearch/0313) Attempt 17: Reduce learning rate from 0.01 to 0.005 to stabilize Q-value
...
| * 0a62e22 (origin/add_agentic_training) Attempt 1: Faster epsilon decay (0.995 vs 0.9993) accelerates the trans
...
| * ffed324 added starting position and results graph but requires more race state monitoring for performance analysis
...
|\  
| * df6694b (origin/grabHistoricalF1Metrics, grabHistoricalF1Metrics) added fastF1 library, and created script for grabbing position overtake data
...
* 637227e implemented slight tweak to make gaps be a function of distance and time, and added corner names on visualisation
...
I acknowledge the use of Claude (Anthropic, \url{https://claude.ai}), specifically the Sonnet 4.6 and Opus 4.7 models, as a writing assistant during the preparation of this report. The tool was used in line with the AMBER category of the University of Leeds Generative AI guidance, in the assistive role of drafting and structuring content, identifying unclear or overlong sentences, and providing alternative phrasings for the author to evaluate. All drafted content was reviewed, edited, and integrated by the author, and the intellectual content of the report (research questions, methodology design, results interpretation, and conclusions) is the author's own work.

I additionally acknowledge the use of the autoresearch framework \citep{karpathy2024autoresearch}, an open-source tool that uses Claude to propose and evaluate hyperparameter configurations, as a development-time experimental tool during Phase~2 of the project. The tool was used to support iterative testing of DQN hyperparameters, with proposed configurations treated as experimental candidates to evaluate rather than as final design decisions. The resulting tuning attempts are recorded in the commit history (Appendix~\ref{appendix:engineering-log}). The author analysed the experimental outputs and selected the final substrate configuration.

The Phase~9 LLM baseline described in Section~\ref{sec:llm-baseline} uses a large language model as a research subject within the experimental programme rather than as a development tool, and is not covered by this acknowledgement.
\end{appendices}
