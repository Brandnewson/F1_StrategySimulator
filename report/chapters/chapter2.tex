...
The methodology has three components, each justified against a separate criterion. The simulator is justified by domain match, the experimental programme by the structure of the research questions, and the engineering practice by the requirements to fulfil the investigation of the research questions.

\section{Simulator design}
\label{sec:simulator-design}

The simulator models a race as a sequence of discrete decision points at overtaking zones, rather than modelling lap-time physics at the second-by-second level \citep{heilmeier2018racesim}. This quasi-static approach, abstracting away continuous car physics between zones and treating each overtake zone as the decision point, keeps each race cheap enough to support multiple sweeps across $\alpha$, game structure, and agent count.
...
\[
R = R_{\text{outcome}} + R_{\text{persistent\_position}} + R_{\text{tactical}}
\]
...
Each experiment runs over five laps at Spa-Francorchamps. Agent composition varies by phase, ranging from one DQN plus the Base Agent up to four DQN agents plus Base, with the exact configuration per phase given in Table~\ref{tab:phases}. The DQN variants referenced throughout the experimental phases (vanilla, double, dueling, rainbow-lite) are introduced in Section~\ref{sec:dqn-foundations} and specified architecturally in Chapter~\ref{chapter3}.

\subsection{Phase-to-RQ mapping}
\label{sec:phase-rq-mapping}

The programme forms a causal ladder where each phase builds on the previous one, so a failure at any earlier step would invalidate downstream claims. Table~\ref{tab:phases} gives the full phase-to-RQ mapping. Phase~9 sits outside the RQ structure as a validity check. If a zero-shot LLM matches a trained DQN agent within this simulator, the MARL investigation's premise is weakened.
...
Phase~2 establishes the DQN-family reliability baseline under a controlled single-agent contract. Each variant (vanilla, double, dueling, rainbow-lite) is trained and evaluated against the fixed Base Agent with identical reward schema, observation contract, stochasticity level, episode budgets, and seeds \citep{henderson2018deep}. Phase~2 produces the per-algorithm stochasticity profiles that inform substrate selection for all subsequent phases. The shared training configuration was determined by systematic hyperparameter tuning prior to Phase~2 on the single-agent contract, with tuning seeds held distinct from those used for held-out stochasticity evaluations \citep{eimer2023hyperparameters}. The full protocol, search space, and resulting values are in Chapter~\ref{chapter3}. No variant is advantaged by an arbitrary configuration choice.
...
\section{Engineering methodology and project management}
...
\subsection{Sprint structure and objective mapping}
...
The full commit graph showing phase-numbered feature branches and the supporting infrastructure work is included in Appendix~\ref{appendix:engineering-log} as evidence of the phased development.